...
##### 1.2 数组
数组是引用类型，数组大小不可变，数组初始化时必须指定长度。
```java
int[] arr = new int[5];
int[] arr = new int[]{12, 21, 21, 22};
int[] arr = {12, 21, 21, 22};
```
获取数组长度可以用arr.length，等于数组里的元素个数，数组最后一个元素的下表是arr.length - 1，因为数组初始元素下标是从0开始的

数组大小不可变，虽然可以给已经定义的数组重新赋值，但实际上这个操作只是给数组变量赋值了一个新的数组的地址
...
- switch：根据某一个表达式的结果，执行不同的分支，一般需要加break，否则会接着下一个case执行， i的值只能是int或者可以自动转换为int的值
  ...
  switch可以匹配字符串，比较的是字符串内容

##### 1.5 循环

`while`，循环次数未知时常用，do...while最少执行一次

`for`，循环次数已知时常用，`for`循环经常用来遍历数组，因为通过计数器可以根据索引来访问数组的每个元素，但是，很多时候，我们实际上真正想要访问的是数组每个元素的值，此时可以使用for each
...
除了数组外，`for each`循环还能够遍历所有“可迭代”的数据类型，包括List`、`Map`等
...
- 常量用final修饰，用大写及下划线命名 `final double MAX_VALUE = 666;`
