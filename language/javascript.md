...
3. **splice**
   ...
   **返回值：**
   ...
   如果删除了元素，则返回一个由被删除元素组成的 **数组**

   如果没有删除元素，返回一个空数组 []：

   不传任何参数时，将不会发生任何修改，即没有删除元素，返回一个空数组 []
...
- 匹配规则
  ...
  2. 特殊符号
     ...
     `\d`：匹配单个数字（digital）字符，相当于 `[0-9]`
     `\w`：匹配单个单词（word）字符，相当于`[a-zA-Z0-9_]`
...
所有语言中的第二步都是明确的，第一和第三步在底层语言中是明确的，但在像 js 这种高级语言中，大部分都是隐含的，js 自动帮我们操作了，js 在定义变量时就完成了内存分配

由于使用内存都是明确的无需我们关心，所以只需要关心内存回收的部分，大多数内存管理的问题都出现在这个阶段，最困难的是如何找到哪些被分配的内存确实已经不再需要了，但这只能是一个近似的过程，因为要知道是否仍然需要某块内存是无法判定的（无法通过某种算法解决），垃圾回收算法实现只能有限制的解决一般问题，下面介绍一些主要的垃圾回收算法和它们的局限性

##### 21.1 引用计数

引用计数是最初级的垃圾收集算法，该算法通过判断 **对象有没有被别的对象引用** 来决定是否回收该对象，当引用为 0 时则回收该对象，IE6、7采用的就是这种算法，但是引用计数有个限制是解决不了循环引用的问题，当两个对象彼此循环引用时，该算法会判断它们至少要一次引用，从而不会回收它们，然而实际上它们被调用之后会离开函数作用域，已经没有用了
