...
JavaScript代码之所以能在浏览器中解析执行是因为浏览器中有JavaScript解析引擎，而每款浏览器的内核都不一样，例如Chrome的是Blink，火狐的是Gecko，苹果是WebKit，而V8是Blink内核内置的JS引擎，性能最好
...
如果想指定包的版本号，可以加`@`，新安装的版本会覆盖旧版本，版本号的含义：第一位数字表示大版本更新，第二位表示功能更新，第三位表示bug修复更新，只要前面的数字更新了，后面的数字需要从零开始
...
就是项目根目录下的`package.json`文件，npm安装包时会自动生成，记录了项目信息，包的配置信息，我们不要手动修改这两个文件，它们由npm自动维护，为什么要有这个文件呢，下面来分析问题

由于三方包往往包含了大量的细小文件，这些细小文件数量众多，我们知道当文件数量太多时，磁盘间的文件传输效率会大大降低，而且众多的文件也增大了项目的体积，往往一个项目本身的源代码所占体积不大，反而三分包占据了项目90%以上的体积，这就非常不利于多人合作时共享项目代码

所以如果我们将包也加入版本管理会非常麻烦，这时有人想到，包本身就在npm共享平台上，大家都可以下载，如果用一个文件来记录包的信息，比如包的名称、版本号等，大家只要有这个记录文件，就能从npm上下载到一模一样的包，这样在多人合作时就可以不用将这个体积巨大的包文件夹加入版本管理，只共享项目本身的源代码和记录文件，完美解决了问题，这个记录文件就是`package.json`文件
...
当我们拿到别人共享的项目文件时，里面的包文件夹已被删除，这时项目文件夹下有`package.json`文件，我们可以直接运行命令安装所有的包

```sh
npm i # 等价于 npm install
```

这条命令没有加包名，是因为它会自动读取配置文件的`dependencies`节点的包名和版本号，一次性将里面记录的包全部下载下来

通过npm安装包的时候可以加参数

##### 8.4 卸载包

当我们不再需要某个包时，仅仅删除包文件夹里的包是不行的，下次还会下载回来，正确做法是使用命令卸载包

```sh
npm uninstall 包名
```

这会将包文件夹里的包和配置文件里的记录同时删除
...
有些包只在开发阶段用得到，项目开发完成之后，上线运行并不需要使用这些包，比如webpack，这时可以将这些包记录在开发依赖`devDependencies`里面，而在开发和上线都需要用到的包则依然放在`dependencies`里面

在安装包的时候添加参数就可以将包放进`devDependencies`里面
...
因为npm是外国网站，下载东西难免会比较慢，而热心的淘宝在国内为我们搭建了一个镜像服务器，每隔一段时间就会同步npm上的包，我们只需要将npm的包安装源切换成淘宝镜像就可以直接到淘宝镜像上面下载包了，这样就提升了我们的下载速度
...
先来看cookie，cookie是存储在客户端的一段不超过4KB的字符串，由键值对组成，还有一系列控制有效期、安全性的可选属性组成，每个域名下的cookie是各自独立的，当客户端发起请求时，会自动把当前域名下的未过期的cookie全部发送到服务器
