...
```
不声明不赋值的变量直接使用会报错，只声明不赋值，变量默认保存特殊值 `undefined`

在 ES6 之前，不声明就直接赋值可以使用，但是变量会变为全局变量

按照惯例，ECMAScript 标识符使用驼峰大小写形式，即第一个单词的首字母小写，后面每个单词的首字母大写，算是最佳实践
...
| BigInt    | 大整数类型，表示非常大的整数，主要用于解决 number 无法安全表示大于`2^53 - 1`的整数的问题 |           |
...
形参默认值，可以指定形参的默认值，不传递参数时就使用形参的默认值，如果想跳过某个形参不传递参数使用默认值可以传递 `undefined`
...
2. `filter()`
   ...
   筛选数组，会创建一个新的数组，新数组中的元素是通过条件筛查后的元素，方法返回一个新数组，原理是遍历数组寻找满足条件的元素并且添加到新数组中
   ...
   var arr = [2, 6, 8, 12, 66, 88]
   var newArr = arr.filter(val => val >= 8)
   console.log(newArr) // Array(4) [ 8, 12, 66, 88 ]
   ```
   
3. `some()`

   查找满足条件的数组元素，返回值是布尔值，如果找到了**第一个**满足条件的元素就返回true并且**终止**查找过程
...
4. `map()`
   ...
   创建一个新数组，其结果是该数组中的每个元素是调用一次提供的函数后的返回值，如果不满足返回条件，则会返回undefined，新数组元素数量与原数组保持一致
...
js的变量提升导致了程序运行的不可预测性、语法过于松散，ES6新增了let关键字用于替换var，具有如下特点

1. let声明的变量只在所处**块级作用域**生效

   ES6之前只有全局作用域和函数作用域，ES6新增了块级作用域，也就是`{}`包括起来的区域，比如if、for都有块级作用域，块级作用域的好处在于防止了内层变量覆盖外层变量

   比如使用var关键字则在块级作用域外面依然可以访问
   ...
   利用此特性可以防止循环变量变为全局变量，使用var声明for循环的循环变量i，for循环结束后，在for循环外面还能访问到i，这是不合理的
...
2. let不存在变量提升
   ...
   在let未推出之前，存在变量提升，就是**把变量提升提到函数的最顶部的地方**，但是**只是提升变量的声明初始化**，并不会把赋值也提升上来，这是不严谨的，而使用let声明变量，变量只能先声明后使用
   ...
   使用var会声明变量并**初始化**（初始化不是赋值）为undefined，然后执行到赋值语句的时候再将其赋值为相应的值，在执行到赋值语句之前打印，自然输出为undefined
   ...
   在初始化之前不能进行词法声明，也就是还未初始化不能使用变量，let没有变量提升，因此连初始化都没有，自然不能输出undefined从而报错

3. 暂时性死区

   **使用let在块级作用域中声明变量后**，这个变量就相当于与块级作用域绑定在了一起，与外界无关了，所以尽管外界声明了相同的变量，但是块级作用域中依然不识别外界的变量
...
ES6新增的const用于声明常量，所谓常量就是一旦被声明其值（内存地址）不能被更改
...
3. 常量的值（内存地址）不可更改
   ...
   虽然数组常量也是常量，但是可以更改其**内容的值**，只是其引用指向的内存地址不能改变
...
>var的作用域为函数作用域（或者叫局部作用域），let和const的作用域是块级作用域，var有变量提升，let和const没有
>
...
ES6 提供了对数组和对象的更简洁的操作方式，结构赋值就是一种，解构赋值可以从数组中逐一提取元素，然后赋值给变量
...
元素一一对应时确实这样解构很方便，但是当元素不是一一对应的时候呢，也只能这样解构，只不过没有被赋值的元素的值将会是 `undefined`
...
对象的解构则是用变量名称去匹配对象的属性名称，匹配上了就将对象属性的值赋值给变量
...
对于对象的解构还可以给变量取别名，从而做到和对象的属性不同名，但是要注意起了别名就只能用别名了
...
1. `Array.from()`
   ...
   console.log(arr) // Array(3) [ "张三", "李四", "王五" ]
   ```
   ...
   var arr = Array.from(arrLike, item => item + '爱吃西瓜')
   console.log(arr) // Array(3) [ "张三爱吃西瓜", "李四爱吃西瓜", "王五爱吃西瓜" ]
   ```

2. `Array.find()`

   找出**第一个**符合条件的数组成员并且返回，没有找到返回undefined
   ...
   console.log(target) // Object { id: 1, uname: "张三" }
   ```

3. `Array.findIndex()`

   找出**第一个**符合条件的数组成员的位置并且返回，没有找到返回-1
...
4. `Array.includes()`
   ...
   ```js
   let arr = [1, 3, 2, 5, 6, 8]
   let result1 = arr.includes(6)
   let result2 = arr.includes(7)
