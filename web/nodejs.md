...
这些操作都需要一定的操作过程，把这些操作过程提取出来制定成为一个规范，大家都去遵循这个规范操作，这样就非常有利于大家的合作交流，不然你是一种拆分引用方式，我是另一种拆分引用方式，到时候我用我的方式去引用你编写的模块，显然是引用不通的

已经有一些标准化组织制定了一些规范，比如CommonJS模块化规范

node遵守的就是CommonJS模块化规范，该规范规定：

1. 模块需要一个表示，于是在每个模块的内部使用module对象来代表当前此模块
2. 使用module对象的exports对象属性对外暴露模块的功能
3. 使用require方法引用模块的功能
...
根据commonjs规范可知，应该使用require方法去加载引用模块

需要注意，使用require方法加载模块时会执行模块中的代码
...
const third = require('third') // 三方模块引用前需要下载（肯定的啦，本地不可能提前下载好所有的三方模块），具体后面介绍
```
...
模块作用域的目的是为了防止全局变量污染，全局变量污染就是不同模块之间的变量产生干扰，比如存在同名变量，在使用script标签引入js文件时，不同文件定义的同名全局变量会被覆盖

既然如此，那岂不是意味着模块的功能无法暴露出去给别人使用了，这样模块就失去了意义，工程师们当然想到了这个问题，这也就是commonjs规范考虑到的，根据该规范，我们可以利用module对象的exports对象属性去导出我们想导出去的成员，这些导出去的成员就可以被别人引用了

当我们通过require方法导入模块时其实得到的返回值就是module对象的exports对象

在默认情况下exports对象是一个空对象，只有当我们将想要导出去的成员交给exports对象时，它才会有值，这些值就是需要导出去的成员
...
在另一个模块中引入module模块，这时再打印导入的exports对象，就可以发现其有属性值了，这些属性值就是我们在module模块中导出的成员
