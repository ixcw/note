...
`object-fit` 是一个 CSS3 属性，用于指定替换元素（如 `<img>` 和 `<video>`）内容在其容器中的显示方式。它主要用于控制图片或视频在指定容器内的适配方式，确保内容在不失真的情况下按需求显示

1. **fill**（默认值）：内容会被拉伸以填满整个容器，不管比例如何，通常图像会变形

2. **contain**：内容按其自身比例缩放，以适应容器的宽高，但不会超出容器范围，如果内容比例和容器不一致，会在容器的空白区域留出空白或显示背景色

3. **cover**：内容按比例缩放以填满容器，不会留有空白，但通常内容会被裁剪

4. **none**：内容不缩放，显示原始尺寸，如果内容比容器大，会超出容器显示；如果小，则显示原始大小，不填满容器

5. **scale-down**：是 `none` 和 `contain` 的结合：内容按比例缩放以适应容器，但只在需要时缩小，避免裁剪或放大

##### 18.2 毛玻璃效果

可以使用 CSS 的 `backdrop-filter` 属性，该属性允许对背景进行模糊和其他图像处理操作
